...
async function getNotice(post) {
  ...
  if (typeof post.id !== 'string') {
    console.warn('[getNotice] invalid notice id:', post.id)
    return post
  }
...
export async function resolvePostProps({
  ...
  post = props?.allPages?.find(p => {
    ...
    const normalizedPostId = typeof p.id === 'string' ? idToUuid(p.id) : null
    const normalizedFullSlug =
      typeof fullSlug === 'string' && fullSlug.length >= 32
        ? idToUuid(fullSlug)
        : null
    ...
    return (
      ...
      (normalizedPostId && slugCandidates.has(normalizedPostId)) ||
   ...
   * 6️⃣ blockMap 兜底
   */
  if (post && typeof post.id === 'string' && post.id && !post?.blockMap) {
    try {
  ...
  } else if (post && post.id && !post?.blockMap) {
    console.warn('[resolvePostProps] skip invalid post.id:', post.id)
  }
...
/**
 * 某些 collection_view_page 页面在 getPage() 结果中不再包含 collection_query，
 * 这里兜底调用 queryCollection 并把 reducerResults 合并回 recordMap。
 */
async function hydrateCollectionQueryIfMissing({
  pageRecordMap,
  collectionId,
  collectionView,
  viewIds,
  ...
  if (!pageRecordMap || typeof collectionId !== 'string') {
    return
  }

  const normalizedCollectionId = idToUuid(collectionId)
  const hasCollectionQuery =
    Object.keys(pageRecordMap?.collection_query?.[collectionId] || {}).length > 0 ||
    Object.keys(pageRecordMap?.collection_query?.[normalizedCollectionId] || {}).length > 0

  if (hasCollectionQuery || !Array.isArray(viewIds) || viewIds.length === 0) {
    return
  }

  pageRecordMap.collection_query = pageRecordMap.collection_query || {}

  const upsertReducerResult = (key, viewId, reducerResults) => {
    if (typeof key !== 'string' || !key) {
      return
    }
    pageRecordMap.collection_query[key] = {
      ...(pageRecordMap.collection_query[key] || {}),
  ...
  for (const viewId of viewIds) {
    ...
    const viewValue = collectionView?.[viewId]?.value
    if (!viewValue) {
      continue
    }
    ...
    try {
      ...
      upsertReducerResult(collectionId, viewId, reducerResults)
      upsertReducerResult(normalizedCollectionId, viewId, reducerResults)
    } catch (error) {
      console.warn(
        '[hydrateCollectionQueryIfMissing] getCollectionData failed:',
        { from, collectionId, viewId },
        error
      )
    }
  }
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  if (typeof SITE_DATABASE_PAGE_ID === 'string') {
  ...
  const rawCollection =
    ...
    (normalizedCollectionId
      ? pageRecordMap.collection?.[normalizedCollectionId]
      : null) ||
    {}
  ...
  // 新结构先统一 block 格式，避免后续解析 page.type 失败
  pageRecordMap.block = adapterNotionBlockMap({ block: pageRecordMap.block || {} }).block
  block = pageRecordMap.block

  await hydrateCollectionQueryIfMissing({
    pageRecordMap,
    collectionId,
    collectionView,
    viewIds,
  ...
  collectionQuery = pageRecordMap.collection_query || {}
  pageRecordMap.block = adapterNotionBlockMap({ block: pageRecordMap.block || {} }).block
  block = pageRecordMap.block
  ...
  // fallback: when collection_query is empty, infer page rows from block map
  if (pageIds?.length === 0) {
    pageIds = Object.values(block)
      .map(item => normalizePageBlock(item?.value))
      .filter(value => {
        if (!value || value?.type !== 'page') return false
        if (value?.parent_table !== 'collection') return false
        return (
          value?.parent_id === collectionId ||
          (normalizedCollectionId && value?.parent_id === normalizedCollectionId)
        )
      })
      .map(value => value.id)
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    const noAccess = rawBlock?.value?.role === 'none' || rawBlock?.role === 'none'

    if (!pageBlock && !noAccess) {
  ...
  // fetch 回来的 block 也可能是新结构，合并前同样适配
