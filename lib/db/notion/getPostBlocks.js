...
// Cache ids confirmed as no-access blocks (role:none) during current build process.
const noAccessBlockIdSet = new Set()
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (typeof id !== 'string' || !id) {
    console.warn('[getPageWithRetry] skip invalid id:', id)
    return null
  }
...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (
      ...
      b.value.children.every(child => child && typeof child === 'object' && child?.value?.id)
    ) {
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  ids = ids
    ...
    .filter(id => !noAccessBlockIdSet.has(id))
  if (ids.length === 0) {
    return {}
  }
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const end = new Date().getTime()
    const responseBlocks = pageChunk?.recordMap?.block || {}

    // Mark no-access blocks to avoid refetching them repeatedly during one build.
    for (const id of batch) {
      const record = getBlockById(responseBlocks, id)
      if (record?.value?.role === 'none') {
        noAccessBlockIdSet.add(id)
      }
    }
